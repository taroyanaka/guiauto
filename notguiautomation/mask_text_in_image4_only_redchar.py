...
class UserAccount:
    ...
    def targets_file(self) -> Path:
        ...
        if not suffix:
                    raise ValueError(f"failed to decode image: {original_name}")
...
def normalize_ocr_text(text: str) -> str:
    return text.replace(" ", "").replace("邵ｲﾂ", "")
...
def mask_red_characters(
    ...
    expanded = cv2.dilate(color_pixels, kernel, iterations=1)
...
def process_image_bytes(
    ...
    mask = mask_red_characters(ocr_image, color_ocr_strength)
    ...
    if output_image_mode == "grayscale":
        ...
        if not ok:
                        raise RuntimeError(f"failed to encode grayscale output: {original_output_name}")
    ...
    if not cv2.imwrite(str(mask_output_path), mask):
                raise RuntimeError(f"failed to write mask output: {mask_output_name}")
...
def parse_multipart_form(headers: Any, body: bytes) -> tuple[str, str, bool, str, list[tuple[bytes, str]]]:
    ...
    if not content_type.startswith("multipart/form-data"):
        raise ValueError("multipart/form-data only")
...
def build_index_html() -> str:
    default_note = "隕九▽縺九ｊ縺ｾ縺励◆" if DEFAULT_INPUT.exists() else "譛ｪ驟咲ｽｮ"
    return f"""<!doctype html>
<html lang="ja">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>譁・ｭ怜・鮟貞｡励ｊ</title>
</head>
<body>
  <main>
    <h1>赤文字マスク</h1>
    <p>input.png: {default_note}</p>
    <p>赤系ピクセルを検出して、膨張後に文字領域を黒塗りします。</p>
  </main>
</body>
</html>"""
