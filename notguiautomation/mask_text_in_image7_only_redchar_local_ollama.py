...
OUTPUT_DIR = APP_DIR / "masked_outputs_ollama_v7"
...
def extract_red_texts_from_ollama(image_path: Path, model_name: str, rows: int, cols: int) -> list[str]:
    """赤文字のみを抽出してテキスト行のリストを返す（bboxは含めない）。
    後段で easyOCR の bbox と照合してマスクを作る。"""
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            # PNG bytes にして Ollama に投げる（テキストのみ要求）
            red_bytes = BytesIO()
            ...
            print(f"[DEBUG] area {row}-{col}: Ollama response (first150) = {text[:150]}")
...
def process_image_file(image_path: Path) -> tuple[Path, Path]:
    ...
    # Ollamaで赤文字のテキストのみを取得（bboxは Ollama に期待しない）
    red_text_list = extract_red_texts_from_ollama(image_path, DEFAULT_MODEL, DEFAULT_ROWS, DEFAULT_COLS)
    print(f"[DEBUG] {image_path.name}: extracted red texts: {red_text_list}")

    # easyOCR で bbox を取得してマスク処理
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract red text strings with Ollama (text-only) and use easyOCR bboxes to mask images.",
