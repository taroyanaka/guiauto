...
def extract_red_texts_with_bbox_from_ollama(
    image_path: Path, model_name: str, rows: int, cols: int
) -> list[dict[str, Any]]:
    """
    ollama_test3.pyと同様の方法で赤色ピクセルのみを抽出してからOllamaで認識し、
    バウンディングボックスを特定する
    戻り値: [{"text": "...", "x": 100, "y": 50, "width": 80, "height": 20, "area_row": 0, "area_col": 0}, ...]
    """
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            # 赤色ピクセルのみを抽出して二値画像を作成（ollama_test3.pyの処理）
            ...
            # OpenCV形式に変換して赤いピクセルの領域を検出
            red_gray = np.array(red_binary)
            contours, _ = cv2.findContours(red_gray, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            # contourを面積でソート（大きい順）
            contours = sorted(contours, key=cv2.contourArea, reverse=True)

            # ollama_test3.pyと同じプロンプト：テキスト出力でのみ取得
            prompt = (
            ...
            for line in text.splitlines():
                ...
                if cleaned and cleaned not in seen:
                    ...
                    # 対応するcontourからバウンディングボックスを取得
                    if line_idx < len(contours):
                        cnt = contours[line_idx]
                        x_local, y_local, w_local, h_local = cv2.boundingRect(cnt)
                    else:
                        # contourがない場合は、赤いピクセル領域全体を使用
                        if contours:
                            all_x = [cv2.boundingRect(c)[0] for c in contours]
                            all_y = [cv2.boundingRect(c)[1] for c in contours]
                            all_x_max = [cv2.boundingRect(c)[0] + cv2.boundingRect(c)[2] for c in contours]
                            all_y_max = [cv2.boundingRect(c)[1] + cv2.boundingRect(c)[3] for c in contours]
                            x_local = min(all_x)
                            y_local = min(all_y)
                            w_local = max(all_x_max) - x_local
                            h_local = max(all_y_max) - y_local
                        else:
                            x_local, y_local, w_local, h_local = 0, 0, chunk_w, chunk_h // 3
                    ...
                    extracted.append({
                        ...
                        "x": left + x_local,
                        "y": top + y_local,
                        "width": max(w_local, 10),
                        "height": max(h_local, 10),
                        "area_row": row,
                        "area_col": col,
                    })
                    print(f"[DEBUG] area {row}-{col}: extracted '{cleaned}' bbox=({x_local},{y_local},{w_local},{h_local})")
                    line_idx += 1
...
def mask_texts_from_ollama_bbox(image: np.ndarray, target_texts_with_bbox: list[dict[str, Any]]) -> np.ndarray:
    """
    Ollamaから取得したバウンディングボックス情報を使ってマスク処理を行う
    """
...
def mask_texts_from_ocr_bbox(image: np.ndarray, target_texts: list[str]) -> np.ndarray:
    """
...
def process_image_file(image_path: Path) -> tuple[Path, Path]:
    ...
    # Ollamaで赤文字テキストを抽出（テキストのみ）
    extracted_red_texts = extract_red_texts_with_bbox_from_ollama(image_path, DEFAULT_MODEL, DEFAULT_ROWS, DEFAULT_COLS)
    red_text_list = [item["text"] for item in extracted_red_texts]
    
    # デバッグ出力：抽出された赤文字
    print(f"[DEBUG] {image_path.name}: extracted red texts: {red_text_list}")
    
    # easyOCRで検出したbboxを使ってマスク処理
    mask = mask_texts_from_ocr_bbox(image, red_text_list)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract red text strings with Ollama (including bbox detection), and output mask/composited images.",
